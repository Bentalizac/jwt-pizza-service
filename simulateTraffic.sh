...
while true; do
  ...
  echo "Login diner..."
  echo $token
  curl -s -X POST $host/api/order -H 'Content-Type: application/json' -d '{"franchiseId": 1, "storeId":1, "items":[{ "menuId": 1, "description": "Veggie", "price": 1.05 },{ "menuId": 1, "description": "Veggie", "price": 1.05 }]}'  -H "Authorization: Bearer $token" > /dev/null
  echo "Bought a pizza..."
  sleep 4
  curl -s -X DELETE $host/api/auth -H "Authorization: Bearer $token" > /dev/null
